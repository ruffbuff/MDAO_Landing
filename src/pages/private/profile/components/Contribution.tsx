...
const styles = {
  calendar: {
    display: 'grid',
    gridTemplateColumns: 'min-content repeat(52, minmax(10px, 18px))',
...
const ContributionOverview: React.FC<ContributionOverviewProps> = ({ initialYear }) => {
    ...
    return (
      <>
        <h2 style={{ textAlign: 'center' }}>Year: {year}</h2>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1rem' }}>
            <button style={styles.button} onClick={handlePrevYear}>
                Previous Year
            </button>
            <button style={styles.button} onClick={handleNextYear}>
                Next Year
            </button>
        </div>
